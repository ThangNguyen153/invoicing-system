...
<div class="row">
    <div style="display: flex;">
        ...
        <div style="float: left;width: 10%; padding: 10px; height: 300px">
            <p>Add new Fruit</p>
            <form action="{{url('add-fruit')}}" method="post">
                @csrf <!-- {{ csrf_field() }} -->

                <input type="text" id="name" name="name" placeholder="enter fruit name here"><br>

                <label for="unit">Choose a category:</label>
                <select name="category_id" id="category_id">
                    @foreach($fruitCategories as $fruitCategory)
                        <option value="{{$fruitCategory->id}}">{{$fruitCategory->name}}</option>
                    @endforeach
                </select>
                <br>
